...
-- TODO: This has a bug where the generated path doesn't always have line of sight to each other, needs to be fixed
function DDLazyThetaStar:FindPath(pathTask)
    ...
    while not (open.num == 0) do
        ...
        -- TODO: Scale iterCount with length of trace
            ...
            for i = 1, neighbourDirections.num do
                ...
                if neighbourPackData then
                    if neighbourPackData & NODE_CLOSED > 0 then
                        -- Only checked already closed nodes as only these can have the best paths to current
        ...
        -- Close current
        gScores[currentId] = currentGScore | parentId | NODE_CLOSED -- We need to recreate this as gscore can have changed
