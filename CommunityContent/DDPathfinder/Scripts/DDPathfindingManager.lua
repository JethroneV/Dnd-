...
function GetNavMeshGenerator(startWorldPos, endWorldPos, pathSettings)
    ...
    for i = 1, #_G[DD_NAVMESH_GLOBAL_ACCESS].generators do
        ...
        local navMeshRes = navMeshGen.GetNumTiles().X * navMeshGen.GetNumTiles().Y * navMeshGen.GetNumTiles().Z
...
local function TickProcessPathTaskQueue()
    local processingTimer = DD_PERF_TIMER.New(true)
    ...
    local processPathTasks = function()
        ...
        if currentPathTask.finished then
            ...
            if pathRes then
                ...
                -- Convert it from tileId to WorldPos
                -- TODO: Move the tile -> worldpos convertion to PathfindingBase
                -- Insert startTile
                local pathNum = 1
                ...
                if checkStartTileId then
                    ...
                    if checkTileX == startTileX and checkTileY == startTileY then
                        ...
                        result.path[pathNum] = currentPathTask.startWorldPos
                        result.path[pathNum].z = navMeshGen.GetNavTileWorldPos(currentPathTask.startTileId).z
                    else
                        result.path[pathNum] = navMeshGen.GetNavTileWorldPos(currentPathTask.startTileId)
                    end
                else
                    result.path[pathNum] = navMeshGen.GetNavTileWorldPos(currentPathTask.startTileId)
                end
                result.pathTileIds[pathNum] = currentPathTask.startTileId

                for i = 1, #pathRes do
                    pathNum = pathNum + 1
                    result.path[pathNum] = navMeshGen.GetNavTileWorldPos(pathRes[i])
                    result.pathTileIds[pathNum] = pathRes[i]
                end

                if pathNum > 1 and currentPathTask.foundPathToGoal then
                    ...
                    if checkEndTileId then
                        ...
                        if checkTileX == endTileX and checkTileY == endTileY then
                            ...
                            result.path[pathNum].x = currentPathTask.endWorldPos.x
                            result.path[pathNum].y = currentPathTask.endWorldPos.y
                        end
                    end
                end

                -- Debug show linetraces between paths
                -- for i = 1, #result.path - 1 do
                --     local startTile = result.pathTileIds[i]
                --     local endTile = result.pathTileIds[i + 1]
                --     local x1, y1, z1 = navMeshGen.UnpackTileCoords(startTile)
                --     local x2, y2, z2 = navMeshGen.UnpackTileCoords(endTile)
                --     -- TODO: Rewrite this to use the callback instead
                --     navMeshGen.NavMeshLineTrace(x1, y1, z1, x2, y2, z2, nil, true)
                -- end
...
function RunPathfindingBenchmark(randomPaths, renderPaths)
    ...
    local startBenchmark = function()
        local benchmarkTask = Task.Spawn(function()
            -- TODO: Pick a random navmeshgen
            if navMeshGen.IsNavMeshReady() then
...
local function TestPath()
    local testTask = Task.Spawn(function()
        ...
        local startPos = Vector3.New(6080, 5300, 2080) -- Castle walls
        --local startPos = Vector3.New(3030, 1410, 0) -- Ground show bad astar
        --local startPos = Vector3.New(-1750, 2950, 0) -- Ground
        --local startPos = Vector3.New(-2450, 7650, 0) -- Ground far away
        --local endPos = Vector3.New(-420, -990, 870) -- Up stairs
        local endPos = Vector3.New(-800, 800, 2150) -- On tower
        --local endPos = Vector3.New(3650, -1400, 0) -- Ground but something in the way
        --local endPos = Vector3.New(5850, 1000, 0) -- Straight Far on ground
        --local endPos = Vector3.New(850, 3500, 0) -- Close on ground
    ...
    if _G.NavMesh or _G.Pathfinder or _G.PathfindingManager or _G[DD_PATHFINDER_GLOBAL_ACCESS] then
        ...
        local wrapper = {
            FindPath = function (startPos, endPos)
                local res = FindPath(startPos, endPos)
                if res then
                    return res.path
                else
                    return nil
        ...
        _G[DD_PATHFINDER_GLOBAL_ACCESS] = script
    end
end
...
Task.Spawn(function()
    ...
    --TestPath()
    --RunPathfindingBenchmark(true, true)
