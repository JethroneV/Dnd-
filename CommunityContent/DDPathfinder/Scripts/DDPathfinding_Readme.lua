--[[
    ...
    When the 4 stages have finished it prints the saving string to the log. Copy the text inside the block (one row per finished NavMesh) and
        paste it in DDNavMeshDataStore.lua inside the marked block at the top of the file (if this is an imported pack, open the file inside the SubFolders directory, not the Imports directory).
    There's a script referens to DDNavMeshDataStore in the DDPathfindingManager template root as a custom property, which you can use for quicker access to it.
    ...
    Start the game now again it should load the navmesh instead of generating a new one.
    ...
    Working with multiple navmeshes:
        ...
        So when should you use multiple navmeshes?
            ...
            - If you want to have a large low resolution navmesh covering the whole map, but you have for example an indoor areas that require higher resolution,
            ...
            - If navmesh generation is too slow then having multiple navmeshes that fits better with the map can improve generation performance.
                This is mostly not recommended because of the pathfind problem between navmeshes, but if you go down this route then try to not make the navmeshes too
                    small and make sure there are no obvious collision on the borders between them between them.
    ...
    DDDontAffectNavMesh:
        ...
        One problem that exist with the navmesh generator is that it needs to run while the game is active, which means there's a possibility for dynamic objects that is in the way.
        ...
        It's important to notice that the script:
        ...
        If you have any ideas to how this workflow could be improved let me know, it works but it's a bit messy. Also it leaves the script even after generation is done, which could have performance implications.
    ...
    - (NOTE, currently has a bug which makes it generate worse paths than intended) DDLazyThetaStar, medium performance, generates pretty good and straight paths most of the time. This is the recommended default algoritm and works well for most cases.
    ...
    WORKING ON (Currently in progress, most likely done pretty soon):
    - Fix bug in LazyTheta* in which it generates paths that doesn't guarantee line of sight to each other, which decrease the quality of it
    - Improve and optimize navmesh linetrace (should result in fewer corner cutting paths)
    - Improve FitPathAgainstNavMeshZ by not restricting it to the grid (will improve paths going up/down slopes)
    ...
    - EASY: Custom tooling for blocking and unblocking parts of the navmesh that have been calculated incorrectly (offline)
    - EASY: Block and unblock paths at runtime
    ...
    - MEDIUM: If a path overlaps a navmesh with higher resolution, switch to it (only generate a path to the overlapping location)
    - HARD: Flying navgrid
    ...
    - VERYHARD: Custom placeable nodes which doesn't need to fit the grid, can be used for special actions path like jumping/falling/flying etc.
    - VERYHARD: Connect multiple navmeshes together automatically
